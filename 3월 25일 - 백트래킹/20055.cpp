...
struct info { //내구도와 로봇 존재 여부
    int power; //해당 위치의 내구도
    bool is_on; //로봇이 있으면 true 없으면 false
};
...
int minusPosition(int n, int pos) { //인덱스 감소
    if (--pos >= 0) { //인덱스 감소시켜서 0보다 크거나 같으면
        return pos; //감소시킨 인덱스 리턴
    }
    return n * 2 - 1; //그게 아니라면 맨끝 위치 리턴
}

void second(int n, int start, int end) {
    int cur = end;
    while (cur != start) { //end - 1 부터 start 까지 검사
        cur = minusPosition(n, cur); //인덱스 하나 감소
        int next = (cur + 1) % (n * 2); //다음 위치
        if (belt[cur].is_on && !belt[next].is_on && belt[next].power) { //현재 위치에 로봇이 있고, 다음 위치에 로봇이 없으며 내구성 1이상인 경우 -> 로봇 옮김
            belt[cur].is_on = false; //옮겼기 때문에 현재는 로봇이 없으므로 false로 업데이트
            belt[next].power--; //로봇이 내려졌으므로 내구도-1
            if (next != end) { //내리는 위치 아닐 경우만
                belt[next].is_on = true; //로봇 옮기기
            }
            if (belt[next].power == 0) { //다음 벨트의 내구도가 0이라면
                zero_power++; //내구도가 0인 칸의 개수를 +1
            }
        }
    }
}
...
void third(int start) {
    if (belt[start].power) { //올리는 위치의 내구도가 0이 아니라면 로봇 올림
        belt[start].is_on = true; //로봇을 올렸으므로 true로 업데이트
        belt[start].power--; //내구도 감소
        if (belt[start].power == 0) { //시작점의 내구도가 0이라면
            zero_power++; //내구도가 0인 칸의 개수를 +1
        }
    }
}
...
int main() {
    ...
    //입력
    cin >> n >> k;
    belt.assign(n * 2, { 0, false }); //n*2만큼 할당
    for (int i = 0; i < n * 2; i++) {
        cin >> belt[i].power; //입력받기
    }
    ...
    while (true) { //무한루프
        ...
        if (belt[end].is_on) { //벨트 회전 후, 로봇이 내리는 위치에 있다면 내리기
            belt[end].is_on = false; //내렸다면 false로 변경
        }
        second(n, start, end); //2번
        third(start); //3번
        step++; //한단계를 진행했으므로 step+1
        if (zero_power >= k) { //0인 내구도의 개수가 k보다 많아지면
            break; //정지
        }
    }
