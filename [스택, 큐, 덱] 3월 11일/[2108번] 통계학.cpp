...
int findMode(int n, vector<int>& arr) {
    ...
    for (int i = 1; i < n; i++) { //첫번째 값은 입력했으므로 두번째 값부터 시작
        // 만약 직전 값과 같은 값이라면
        if (arr[i] == arr[i - 1]) {
            count[current_idx].second++; //개수++
        }
        else {
    ...
    if (count[0].second == count[1].second) {
        return count[1].first; //두번째로 작은 값 리턴
    }
    return count[0].first; //최빈값 리턴
}
...
int main() {
    ...
    for (int i = 0; i < n; i++) { //n만큼 반복
        cin >> arr[i]; //수를 int형 vector에 저장
    }
    sort(arr.begin(), arr.end()); //오름차순 정렬
    // 산술평균
    int sum = accumulate(arr.begin(), arr.end(), 0); //accumulate함수) 배열의 합 구하기. 처음부터(begin()) 끝까지(end()) 초기값은 0
    // round() 함수를 사용할 수도 있어요. 다만 -0이 나오지 않도록 조건문을 사용해야 합니다.
    cout << floor((double)sum / n + 0.5) << '\n'; //floor:내림함수 / 소수점 첫째 자리에서 반올림 위해 0.5 더하기.
