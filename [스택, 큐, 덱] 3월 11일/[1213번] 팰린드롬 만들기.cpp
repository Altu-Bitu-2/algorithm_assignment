...
vector<int> countAlphabet(string text) {
    ...
    return count; //각 알파벳 개수 리턴
}

string makePalindrome(vector<int>& count) { 
    // 팰린드롬을 만들어 리턴하는 함수.
    string part1 = "", part2 = "", part3 = ""; //part1: part3과 대칭, part2: (있다면)가운데 놓을 홀수인 알파벳 1개 ,part3: part1과 대칭

    for (int i = 0; i < SIZE; i++) {  //A-Z까지
        // 만약 알파벳의 수가 홀수라면
        if (count[i] % 2 == 1) {
            // 이미 가운데 글자가 찼다면 더 이상 불가능
            if (part2.size() == 1) {
                return "I'm Sorry Hansoo"; //불가능 메세지 출력
            }
            // 가운데 글자가 비어있다면 할당
            part2 = (char)(i + 'A');
        }
        // 남은 수의 절반을 나누어 part1과 part3에 이어 붙인다
        for (int j = 0; j < count[i] / 2; j++) { //알파벳 개수의 절반만큼 붙임
            part1 += (char)(i + 'A'); //들어온 순서대로 알파벳 붙이기
            part3 = (char)(i + 'A') + part3; //앞으로 알파벳 붙이기 (늦게 들어올수록 앞에/ part1과 대칭)
        }
    }

    return part1 + part2 + part3; //완성된 팰린드롬 문자열 리턴
}
...
int main() {
    ...
    // 연산 + 출력
    vector<int> count = countAlphabet(input);  //알파벳 개수 세기
    cout << makePalindrome(count); //팰린드롬 문자열 만들기
