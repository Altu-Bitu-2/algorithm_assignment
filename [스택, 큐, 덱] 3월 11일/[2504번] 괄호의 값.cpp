...
int calc(string s) {
    ...
    int ans = 0, temp = 1;
    for (int i = 0; i < s.length(); i++) { //string(입력값)의 길이만큼 반복
        switch (s[i]) {
        case '(': case '[': //여는 괄호일 경우
            st.push(s[i]);  //push
            temp *= num[s[i]];  //안에 있는 것은 곱하기 연산
            break;
        case ')': case ']': //닫는 괄호일 경우
            if (st.empty() || st.top() != bracket[s[i]]) { 
                //길이만큼 돌지 않았는데도 스택이 비어있거나, 맨 위의 원소가 입력된 원소와 쌍이 아닐 경우,
                return 0; //입력이 올바르지 못하므로 0출력
            }
            if (s[i - 1] == bracket[s[i]]) { //이전 값이 입력된 괄호(닫힌괄호)와 쌍인 열린 괄호였다면
                ans += temp; //분리된 괄호열이므로 더하기 연산
            }
            temp /= num[bracket[s[i]]]; //이미 값을 더한 경우이므로 나누기
            st.pop(); //쌍이 완성된 경우 pop
            break;
        }
    }
