...
int lis(int n, vector<int>& arr) {
    ...
    for (int i = 1; i < n; i++) { 
        for (int j = 0; j < i; j++) {   //부분수열
            if (arr[i] > arr[j]) { //증가하는 관계라면
                dp[i] = max(dp[i], dp[j] + 1); //최댓값 구하기
            }
        }
...
int lisAdv(int n, vector<int>& arr) {
    ...
    int idx = 0;    //가장 긴 증가하는 부분 수열의 길이

    for (int i = 0; i < n; i++) {
        for (int j = idx; j >= 0; j--) {
            if (arr[i] > dp[j]) { //증가하는 관계라면
                dp[j + 1] = arr[i];   //수열 값 추가
                if (j == idx) { //최장 길이 갱신
                    idx++; //수열 길이증가
                }
                break;
...
int main() {
    ...
    //입력
    cin >> n;
    vector<int> arr(n, 0);  //n 크기의 수열을 저장할 배열
